...
`data/sms/evidence/keep/A_test.csv`, and
`data/voice/evidence/keep/F_train.csv`.

All released train, validation, test, and challenging splits are read from:

- `data/sms/evidence/keep/{A,B,C,D}_{train,validation,test,challenge}.csv`
- `data/voice/evidence/keep/{E,F,G}_{train,validation,test,challenge}.csv`
...
than regenerating length profiles. Lower-level suite runners can regenerate
profiles only when invoked directly without `--skip-length-profiles`.
