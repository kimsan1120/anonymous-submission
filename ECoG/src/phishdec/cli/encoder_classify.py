...
from phishdec.encoder.pipeline import run_encoder_from_config
...
def main() -> int:
    ...
    exp_name = _get_cfg(cfg, "exp_name", "encoder_classify")
    ...
    success = False
    try:
        run_encoder_from_config(cfg=cfg, run_dir=run_dir)
        success = True
    finally:
        if success and use_running:
